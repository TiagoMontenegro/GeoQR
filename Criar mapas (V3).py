...
def generate_maps():
    ...
    if file_paths:
        for file_path in file_paths:
            ...
            base_filename = os.path.splitext(os.path.basename(file_path))[0]  # Save without ".csv"

            # Get the directory of the selected CSV file
            csv_dir = os.path.dirname(file_path)

            # Construct the save path in the same directory as the main script
            save_path = os.path.join(csv_dir, f"{base_filename}.html")
...
# Fetch image from server for the background app
